...
export function InviteList() {
  ...
  const inputStyle = { padding: '10px 14px', fontFamily: 'system-ui, sans-serif', fontSize: '13px', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 0, outline: 'none', background: 'var(--color-vcx-beige-light)' }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <button onClick={() => setShowCreate(!showCreate)} style={{ padding: '8px 20px', fontFamily: 'system-ui, sans-serif', fontSize: '13px', fontWeight: 600, color: 'var(--color-vcx-beige)', background: 'var(--color-vcx-dark)', border: 'none', borderRadius: 0, cursor: 'pointer' }}>직접 초대</button>
      </div>
      ...
      {showCreate && (
        <form onSubmit={handleCreate} style={{ padding: '20px 24px', background: 'var(--color-vcx-beige)', marginBottom: '24px', borderLeft: '2px solid var(--color-vcx-gold)' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-end' }}>
            ...
            <div>
              <label style={{ display: 'block', fontFamily: 'system-ui, sans-serif', fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-vcx-sub-4)', marginBottom: '6px' }}>등급</label>
      ...
      {loading ? (
        ...
        <p style={{ fontFamily: 'system-ui, sans-serif', fontSize: '14px', color: 'var(--color-vcx-sub-4)' }}>초대가 없습니다</p>
      ) : (
        <div>
          {data.map((inv) => (
            <div key={inv.id} style={{ padding: '20px 24px', borderBottom: '1px solid rgba(0,0,0,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '6px' }}>
                  <span style={{ fontFamily: 'system-ui, sans-serif', fontSize: '14px', fontWeight: 600, color: 'var(--color-vcx-dark)' }}>{inv.email}</span>
                  <StatusBadge status={inv.status} />
                  <span style={{ fontSize: '11px', fontFamily: 'system-ui, sans-serif', color: 'var(--color-vcx-gold)', border: '1px solid var(--color-vcx-gold)', padding: '2px 8px' }}>{inv.member_tier === 'core' ? 'CORE' : 'ENDORSED'}</span>
                </div>
              ...
              {inv.status === 'pending' && (
                <button onClick={() => handleRevoke(inv.id)} style={{ padding: '8px 16px', fontFamily: 'system-ui, sans-serif', fontSize: '13px', color: 'var(--color-vcx-sub-4)', background: 'transparent', border: '1px solid rgba(0,0,0,0.15)', borderRadius: 0, cursor: 'pointer' }}>취소</button>
              )}
            </div>
          ))}
          {total > 20 && (
