...
Major	이름/LinkedIn 중복 수집	초대 수락 시 이미 받은 정보를 온보딩에서 재요청
...
1. 비즈니스 전략
1.1 핵심 명제
시대의 인재를 모은다 — 회사에 한 명 있을까 말까한 사람들이 모이는 곳. 이들이 머무는 이유를 설계하면, 헤드헌팅은 자연스럽게 따라온다.

1.2 비즈니스 구조
VCX의 비즈니스는 3개 레이어로 구성된다:

레이어	기능	수익 기여
채용정보 큐레이션 (Hook)	초개인화 채용시장 데이터 → 인재 유입 & 리텐션	간접 (인재 확보)
커뮤니티 라운지 (Sticky)	익명 커뮤니티 → 인재가 머무는 이유 제공	간접 (이탈 방지)
채용 연결 (Revenue)	CEO Coffee Chat + Peer Coffee Chat → 수수료 수익	직접 (핵심 수익)

1.3 수익 모델
수익원	구조
성사 수수료 (주수익)	채용 성사 시 연봉의 25% — CEO Coffee Chat, Peer Coffee Chat 모두 적용
Self Introduction Reward	멤버 본인 직접 추천 → 채용 성사 시 고정 보상 지급 (인재 신뢰도 강화 목적)
Peer Referral Reward	멤버가 동료 추천 → 채용 성사 시 높은 보상 (소싱·보증 노력 반영)
...
CEO/Founder/C-Level이 세션 생성 → 멤버가 신청 → CEO가 선택. 세션 생성 시 헤드헌팅 계약 조건 동의.
...
CEO Coffee Chat — 역방향 채용의 무게감
•	세션 카드: CEO 이름/회사/시리즈 정보를 대형 타이포그래피로 — 지원서가 아닌 초대장 느낌
•	신청 전 계약 동의: 인라인 모달, '이 대화에서 채용이 발생할 경우 VCX 수수료 적용에 동의합니다'
...
한 줄 요약
채용정보 큐레이션으로 유입하고, 커뮤니티 라운지로 머물게 하고, CEO Coffee Chat으로 수익화한다.
지금 가장 중요한 것은 30명을 모으는 것이 아니라, 30명이 머무는 이유를 설계하는 것이다.
