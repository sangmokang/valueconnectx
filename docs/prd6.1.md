# ValueConnect X — PRD v6.1 (2026-04-19 개정, rev 3)
...
> 기준일: 2026-04-19. 저자: Prometheus (Strategic Planning). Review: Critic loop (rev 2 — 스키마/경로 불일치·운영 가드 수정, rev 3 — 비전 문구 정제·커뮤니티 카테고리 재설계·Concierge Onboarding 신설·ReAct 에이전트 프로토콜 도입).
...
| C11 **(rev 3)** | §1.1, §1.2 | 비전 문구가 추상적, Sticky 레이어 설명이 난해 | **"함께 성장하는 커뮤니티" 문구 추가 + Sticky 의미를 (a) 사회적 증명 / (b) 피어 대화로 명시 구체화** | 리뷰 피드백 — 원문 문장만으로는 이해 불가했던 Sticky 로직 분해 |
| C12 **(rev 3)** | §1.5 커뮤니티 | DB CHECK 값과 본문 설명 불일치(독서·가벼운이야기 vs salary·company_review) + 자유글 칸 부재 | **8 카테고리 재설계 — 기존 6 정리(`salary`→`compensation` rename) + `learning`/`free` 2개 신설, migration 030** | 강제 분류 장벽이 글 수 감소로 이어지는 failure mode 방어 |
| C13 **(rev 3)** | Feature 1.1 신규 | LinkedIn 필수 전제가 초특급 인재(박지성·안정환급)를 시스템 밖으로 밀어냄 | **Concierge Onboarding 경로 신설 — `onboarding_channel='concierge'` + sealed-envelope PW + 2FA 강제, migration 031** | "LinkedIn 없음"은 문제가 아니라 VIP 신호. 수동 1:1 온보딩을 정식 프로세스화 |
| C14 **(rev 3)** | §3.2 React 구성 원칙 | 원칙 table뿐, AI 에이전트가 어떻게 디자인 시스템을 지킬지 프로토콜 없음 | **ReAct (Reasoning + Acting) 루프 공식 도입 — Thought→Action→Observation→Finish + `eslint-plugin-vcx-brand` 검증 Observation 자동화** | v6.0 "서브페이지 디자인 붕괴"의 근본 원인(에이전트의 토큰 무시)을 구조적으로 차단 |
...
우리가 만드는 것은 단순한 채용 플랫폼이 아니라 **시대의 인재들이 함께 성장하는 커뮤니티** 다. 동급의 사람들이 서로의 존재를 확인하고, 솔직한 경험을 나누고, 필요할 때 연결된다 — 배움·신뢰·기회가 이 순서로 자연스럽게 일어나는 장(場).
...
| **Sticky** — 커뮤니티 라운지 + Member Directory | 인재가 떠나지 않는 이유 = **(a) 사회적 증명** ("나와 동급의 사람들이 실재한다"는 확인 — Directory Level 0 집계 + Level 1 마스킹 카드) + **(b) 피어 대화** (익명 라운지에서 동료 리더들의 솔직한 경험담·질문·답변을 읽고 쓰는 경험) | 이탈 방지, LTV 상승 | 간접 |
...
### 1.5 커뮤니티 라운지 — 2026-04-02 recode 완료, v6.1에서 카테고리 재설계

**현 상태**: `community_posts.category` DB CHECK 값은 `career | leadership | salary | burnout | productivity | company_review` (migration 009). `company_review`는 privacy model(`015_vcx_privacy_model.sql`)로 기업 사용자 시점 작성자 마스킹 적용됨.

**v6.1 재설계 배경**: 리뷰 중 (a) PRD 본문 설명(독서·이직·회사생활·리더십·생산성·가벼운 이야기)과 (b) DB CHECK 실제 값(career/leadership/salary/burnout/productivity/company_review)이 **일치하지 않음**을 발견. 또한 어느 카테고리에도 속하지 않는 자유로운 글쓰기 칸이 없어 고품질 게시글이 "강제 분류" 때문에 유실될 가능성 존재.

**v6.1 최종 카테고리 (8개 = 기존 6 + 신규 2, migration 030)**:

| slug | 라벨 | 의도 | 프라이버시 |
|------|------|------|-----------|
| `career` | 커리어·이직 | 이직 고민, 커리어 전환, 협상 | 익명 기본 |
| `leadership` | 리더십 | 팀 빌딩, 의사결정, 피드백 | 익명 기본 |
| `compensation` | 보상·처우 | 연봉·스톡옵션·베네핏 (기존 `salary` 리네임 — 보너스/이퀴티 포함 의미 확장) | 익명 강제 |
| `burnout` | 번아웃·워라밸 | 정신 건강, 회복, 워라밸 | 익명 강제 |
| `productivity` | 생산성·도구 | 생산성 시스템, 툴, 루틴 | 선택 |
| `company_review` | 회사 리뷰 | 재직/퇴직 회사 솔직 리뷰 | 익명 강제 + 기업 사용자 시점 마스킹 (015) |
| **🆕 `learning`** | 학습·독서 | 책·아티클·강의 추천·후기 (PRD 본문에 있었으나 DB 누락분 보충) | 선택 |
| **🆕 `free`** | 자유 | 위 카테고리에 맞지 않는 모든 글. 사담·질문·잡담·이벤트 | 선택 |

**왜 `free`가 필요한가**: 강제 분류는 "이 글은 어디에 써야 하지?" 고민 → 작성 포기 → 글 수 감소의 전형적 failure mode. Invite-only 커뮤니티에서 **글 수 자체가 생명선**이므로 분류 장벽을 낮추는 것이 최우선. `free` 카테고리는 기본값 정렬에서 후순위로 두어 "진짜 저질 사담"의 홍수는 막는다.

**마이그레이션 계획 (migration 030, Phase 1 Week 2)**:
1. `community_posts.category` CHECK constraint 교체: 위 8개 slug 허용
2. 기존 데이터 중 `salary` → `compensation` UPDATE (rename)
3. 기존 `company_review` privacy 규칙 유지 (015)
4. 프론트 카테고리 탭 순서: `career → leadership → compensation → burnout → productivity → learning → company_review → free`

**수락 기준**:
- [ ] migration 030 production 적용 및 기존 `salary` 글이 `compensation`으로 자동 이관됨
- [ ] `free` 카테고리 UI에서 "자유롭게 써보세요 — 어디에도 안 맞는 글을 위한 공간" 헬퍼 문구 노출
- [ ] 재설계 이후 1개월간 `free` 카테고리 월간 글 수 ≥ 전체의 15% (적정 활용 증명)
...
### Feature 1.1 (신규): Concierge Onboarding — LinkedIn 없는 초특급 인재용 VIP 경로

**왜 필요한가**: 진짜 박지성·안정환 급의 인재(시대의 상징적 인물, 은퇴 후 2nd career, 시니어 C-level 등)는 **LinkedIn 계정이 없을 가능성**이 실제로 있다. 기본 온보딩 흐름(LinkedIn URL 필수)은 이들을 시스템 밖으로 밀어낸다. "그런 분들이 있다는 사회적 증명"이 네트워크 초기 가치의 핵심인데, 폼 장벽 하나로 유실하면 전략 자체가 무너진다.

**해결 원칙**: "링크드인이 없다"는 **문제가 아니라 신호**다 — 그 정도 인재는 우리가 직접 찾아가 모셔와야 한다. 호텔 컨시어지·사설 은행(PB) 급의 **1:1 수동 온보딩(Concierge Path)** 을 정식 프로세스로 설계한다.

**프로세스 (7 단계)**:

| 단계 | 담당 | 산출물 |
|-----|------|-------|
| 1. **식별·접촉** (오프라인) | 창업자/파트너 | 에이전트·매니저·지인을 통해 대면 또는 소개. "초대장"은 물리적 카드 + 개별 email 동시 전달 |
| 2. **VCX Concierge 계정 생성** (service_role) | Admin | `vcx_members` INSERT with `onboarding_channel='concierge'`, `linkedin_url=NULL`, `external_verification_doc_url=<내부 링크>` |
| 3. **임시 ID/PW 봉인 전달** | Admin | 1회용 랜덤 12자 PW를 `vcx_concierge_credentials` 테이블에 `bcrypt` 해시 저장. 실물은 봉인 봉투 또는 signal/개인 채널로 전달 (email 금지) |
| 4. **최초 로그인 강제 PW 변경 + 2FA 등록** | 본인 | `first_login_completed_at`, `mfa_enrolled_at` 기록 |
| 5. **프로필 작성 대행 (선택)** | Admin 또는 본인 | 본인이 원하면 비서·에이전트 또는 운영팀이 대필(ghost-write)하고, 본인이 최종 승인 버튼만 눌러 저장 |
| 6. **Tier 배지 부여** | Admin | `member_tier='core'`, `verified_by='concierge'`, 프로필에 "운영팀 직접 검증" 배지 노출 |
| 7. **후속 관리** | Partner | 분기 1회 오프라인 체크인, 커피챗 매칭 시 상대방에게 알림 자동 발송 안 함 (asymmetric privacy — 본인 동의 후에만 상대방에게 존재 노출) |

**데이터 모델 (migration 031, Phase 1)**:

```sql
-- vcx_members에 컬럼 추가
ALTER TABLE vcx_members
  ADD COLUMN onboarding_channel TEXT NOT NULL DEFAULT 'invite'
    CHECK (onboarding_channel IN ('invite','concierge','self_signup_blocked')),
  ADD COLUMN external_verification_doc_url TEXT,     -- 내부 문서/사진/에이전트 확인서
  ADD COLUMN verified_by TEXT,                        -- 'linkedin' | 'concierge' | 'peer_referral'
  ADD COLUMN first_login_completed_at TIMESTAMPTZ,
  ADD COLUMN mfa_enrolled_at TIMESTAMPTZ;

CREATE TABLE vcx_concierge_credentials (
  member_id       UUID PRIMARY KEY REFERENCES vcx_members(id) ON DELETE CASCADE,
  pw_hash         TEXT NOT NULL,                     -- bcrypt, 1회용 초기 PW
  issued_by       UUID NOT NULL REFERENCES vcx_members(id),
  issued_at       TIMESTAMPTZ NOT NULL DEFAULT now(),
  delivered_via   TEXT NOT NULL CHECK (delivered_via IN ('sealed_envelope','signal','in_person','encrypted_email')),
...
> **FK 주의**: `issued_by`는 `vcx_members(id)`를 참조한다. VCX에서 **Admin은 `vcx_members.system_role IN ('admin','super_admin')`인 멤버** 이며 `vcx_corporate_users`와 무관하다(migration 001 기준). 즉 발급자는 "기업 사용자"가 아니라 "관리자 권한을 가진 멤버"다.

**보안·운영 가드**:
- 초기 PW는 **이메일 금지** (탈취 위험). sealed envelope / Signal / 대면 전달만 허용. 배송 수단은 `delivered_via`에 기록
- **72시간 만료 정책**: `issued_at + 72h` 내 최초 로그인 없으면 credential invalidate + 재발급 필요
- 2FA 필수 — 일반 멤버는 optional이지만 concierge 경로는 강제
- Admin 전용 감사 로그: 누가·언제·누구에게 발급했는지 `vcx_access_audit_log`에 자동 기록 (Phase 2 `027` 의존 — Phase 1엔 수기 Notion 로그)
- **식별 검증 문서**: LinkedIn 대체물(언론 기사 URL, 저서 ISBN, 공식 프로필 페이지, 에이전트 확인서 등)을 `external_verification_doc_url`에 private storage 링크로 저장

**UI/UX**:
- 본인은 일반 멤버와 동일한 UI 사용. "concierge" 배지는 내부에만 보이고 외부 노출 X (본인의 VIP status를 과시하지 않는 것이 Quiet Luxury 원칙에 부합)
- 본인에게는 전용 "컨시어지 매니저" 연락처(Admin 1인 지정)가 프로필 내 개인 섹션에 노출 — 문제 발생 시 즉시 연락 가능

**수락 기준**:
- [ ] migration 031 production 적용 (`onboarding_channel`, `vcx_concierge_credentials`)
- [ ] Admin UI에서 concierge 계정 발급 플로우 (`/admin/concierge-invite`) 가능 — PW 생성 → 해시 저장 → 운영 알림
- [ ] 최초 로그인 시 PW 변경 + 2FA 강제 등록 플로우 동작 (Playwright E2E)
- [ ] 72시간 미로그인 시 credential invalidate cron (`pg_cron` daily)
- [ ] 외부 문서에 "박지성·안정환 예시" 같은 실명 언급 없음 (프라이버시)
...
### 3.2 React 구성 원칙 (+ ReAct 에이전트 프로토콜)

#### 3.2.1 React 컴포넌트 구성 원칙
...
#### 3.2.2 ReAct — LLM이 디자인 시스템을 지키게 만드는 에이전트 프로토콜

> **이 절의 목적**: Claude Code·Codex 등 AI 에이전트가 VCX UI 코드를 생성할 때 **디자인 시스템을 무시하고 하드코딩(`#3B82F6`, `padding: 13px`)을 쏟아내는 failure mode를 구조적으로 차단**한다. v6.0 §0.1 "서브페이지 디자인 붕괴"의 근본 원인 중 하나가 에이전트의 토큰 무시였다.

##### (a) 배경 — ReAct란

- **논문**: Yao et al., "ReAct: Synergizing Reasoning and Acting in Language Models" (ICLR 2023)
- **핵심 아이디어**: LLM의 **Reasoning(추론)** 과 **Acting(행동)** 을 단일 루프에서 교차. `Thought → Action → Observation → Thought → ...` 의 폐쇄 루프로, 중간 관찰 결과가 다음 추론에 반영되어 환각·맹목 실행·중간 이탈을 억제.
- **대안 대비 장점**: Chain-of-Thought(생각만)는 환각·한 번 틀리면 끝까지 끌려감 / Action-only(도구만)는 "왜 쓰는지" 없음. 둘의 단점을 교차로 상쇄.

##### (b) VCX 디자인 시스템에 이식하는 4단계 루프

모든 UI 생성·수정 작업은 아래 루프를 **반드시** 따른다. `.cursor/rules`, `CLAUDE.md`, `skills/SKILL-design-system.md` 모두 동일 프로토콜을 명시한다.

```
┌─────────────────────────────────────────────────────────────┐
│ 1. Thought: 요구사항을 Atom/Molecule/Organism 단위로 분해  │
│ 2. Action:  토큰·컴포넌트 카탈로그 조회                    │
│             (Read src/constants/site.ts, Glob ui/*.tsx)    │
│ 3. Observation: 사용 가능한 토큰·props·variants 파악        │
│ 4. Thought: 존재하면 → variant 조합. 없으면 → 최소 새 atom │
│ 5. Action:  토큰 참조로 코드 생성 (하드코딩 금지)          │
│ 6. Action:  자가검증 — validate_design_system(output)      │
│ 7. Observation: violation 감지 시 2번으로 복귀             │
│ 8. Finish:  lint·build 통과 후 최종 코드                   │
└─────────────────────────────────────────────────────────────┘
```

##### (c) 에이전트가 지켜야 할 구체 규약 (VCX 특화)

| 규약 | 적용 도구·검증 |
|------|--------------|
| **Thought 강제 기록** | 모든 편집 세션에 "왜 이 atom/variant를 선택했는가" 를 conversation에 남김 (CLAUDE.md의 continuation enforcement 규칙과 결합) |
| **토큰 우선 Read** | 코드 작성 전 `src/constants/site.ts` `DESIGN_TOKENS` 조회. `Grep` / `Read` tool 우선 |
| **카탈로그 우선 Glob** | 새 컴포넌트 생성 전 `src/components/ui/*.tsx` · `src/components/{domain}/*.tsx` 에 유사 atom/organism 존재 여부 확인 |
| **하드코딩 금지 Action** | `#[0-9A-Fa-f]{3,8}` / `px` 단위 임의 숫자 / `rounded-*` 클래스는 ESLint 레벨에서 error |
| **검증 Observation** | `npm run lint` + 커스텀 규칙 `no-hardcoded-color` / `no-raw-spacing` / `no-gold-text` / `no-rounded-utility` 통과해야 Finish |
| **계층 경계 위반 금지** | Page에서 Atom 스타일 override 금지. 필요 시 Molecule/Organism 추출. (Atomic Design 원칙 §3.1) |
| **실패 시 Self-correction** | Observation이 violation을 돌려주면 즉시 2번으로 복귀. 3회 이상 동일 violation이면 사람에게 에스컬레이트 |

##### (d) ESLint 커스텀 플러그인 `eslint-plugin-vcx-brand` (Phase 1 Task #10 — **미구현, 신설 예정**)

ReAct 루프의 **Observation 단계 자동화 엔진**. 에이전트가 제출한 코드를 즉시 검증하고 에러 메시지로 다음 Thought에 반영되게 한다.

> **현재 상태**: 이 플러그인은 리포지토리에 아직 존재하지 않는다. Phase 1 Task #10의 산출물이다. Task #10 완료 이전의 ReAct 루프 Observation 단계는 수동 검증(PR 리뷰·코드 리뷰어 확인)으로 대체한다. Task #10 완료 시점부터 아래 규칙이 `npm run lint`로 자동 강제된다.

| 규칙 | 패턴 | 에러 메시지 |
|------|------|-----------|
| `no-hardcoded-color` | `/#[0-9A-Fa-f]{3,8}\b/` in `className` or inline style | "Use DESIGN_TOKENS.colors.* — see src/constants/site.ts" |
| `no-raw-spacing` | `padding:\s*\d+px`, `margin:\s*\d+px` | "Use Tailwind spacing scale (p-1, p-2, ...)" |
| `no-gold-text` | `text-(\[#c9a84c\]\|accent-gold)` | "Gold is for dividers/icons only (2.5:1 contrast). Use text-neutral-* for text." |
| `no-rounded-utility` | `rounded-(?!none\b)` | "VCX 전역 border-radius: 0. rounded-* 클래스 금지 (CLAUDE.md anti-pattern)" |
| `no-tailwind-config-file` | file named `tailwind.config.(ts\|js)` | "Tailwind v4 is CSS-first; use @theme in globals.css instead" |
| `no-fee-percent` | `/\b\d{1,2}\s*%/i` with nearby `placement|수수료|커미션` | "수수료율 UI 노출 금지 (§1.3 원칙)" — §7 O7과 일치 |

##### (e) 기대 효과 (v6.0 붕괴 원인 대비 구조적 해소)

| v6.0 Failure Mode | ReAct 프로토콜 적용 후 |
|-----------------|--------------------|
| 에이전트가 `<button style={{background:'#3B82F6'}}>` 생성 | `no-hardcoded-color` violation → 다음 Thought에서 `DESIGN_TOKENS.colors.accentGold` 조회 → `<Button variant='primary'>` 로 수정 |
| 동일 도메인에 중복 atom 난립 (MemberBadge, TierBadge, StatusBadge 각기 생성) | 카탈로그 우선 Glob → 기존 `Badge` 발견 → variant 추가로 해결 |
| Page에서 atom 스타일 오버라이드 | 계층 경계 규칙 위반 감지 → Molecule/Organism 추출로 승격 |
| "왜 이 색을 썼는지" 추적 불가 | Thought 로그로 의사결정 감사 가능 (commit 메시지에 ReAct 루프 요약 포함) |

##### (f) 확장 계열 (참고, Phase 3 재평가)

- **Reflexion** (Shinn et al., 2023): ReAct + self-reflection. 실패 trajectory를 메모리에 저장 → 다음 시도에 반영. Phase 3 `match_features` 학습 로직과 결합 검토
- **Plan-and-Execute**: Thought를 고수준 plan / 저수준 execute로 분리. 대규모 리팩터에 유리
- **Tree-of-Thoughts**: Thought 분기 탐색 → 최적 경로 선택. 디자인 변형 탐색에 적용 가능
- **ReWOO**: Observation 의존성 최소화로 토큰 비용 절감. Claude API 비용 가드(§4.3)와 연계

> **구현 산출물** (Phase 1 Task #10으로 스코프 확장): (1) `eslint-plugin-vcx-brand` NPM 로컬 패키지 + (2) `skills/SKILL-design-system.md` 에 위 ReAct 루프 명시 + (3) CLAUDE.md에 "UI 편집 전 토큰·카탈로그 우선 Read" 규칙 추가.
...
| 10 | `eslint-plugin-vcx-brand` 로컬 패키지 — `no-hardcoded-color` / `no-raw-spacing` / `no-gold-text` / `no-rounded-utility` / `no-fee-percent` + ReAct 프로토콜 `skills/SKILL-design-system.md` 작성 (§3.2.2) + Mixpanel 최소 이벤트 | — | FE | 2d | MUST |
| 11 | 커뮤니티 카테고리 재설계 migration + 프론트 탭 순서 변경 (§1.5) | 030 | BE+FE | 1d | MUST |
| 12 | Concierge Onboarding — `onboarding_channel` 컬럼 + `vcx_concierge_credentials` + Admin 발급 UI + 72h 만료 cron (Feature 1.1) | 031 | BE+FE | 3d | MUST |
...
| 030 | `030_vcx_community_categories_v2.sql` | 커뮤니티 카테고리 재설계: `salary`→`compensation` rename + `learning` / `free` 추가 + CHECK 교체 (§1.5) |
| 031 | `031_vcx_concierge_onboarding.sql` | `vcx_members`에 `onboarding_channel` / `verified_by` / `mfa_enrolled_at` 등 + `vcx_concierge_credentials` (Feature 1.1) |
